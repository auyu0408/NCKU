...
int main(int argc,char *argv[])
{
	...
	*/
	...
	if (size != 1) {
		...
		// Send Upper Bound
		MPI_Scatterv(BMPData[0], bound_sendcounts, start_addr, MPI_RGBTRIPLE, Buffer_upper[0], bmpInfo.biWidth, MPI_RGBTRIPLE, 0, MPI_COMM_WORLD);
		// Send lower Bound
		MPI_Scatterv(BMPData[0], bound_sendcounts, lower_bound_displs, MPI_RGBTRIPLE, Buffer_lower[0], bmpInfo.biWidth, MPI_RGBTRIPLE, 0, MPI_COMM_WORLD);
	}
		...
		for(int i = 0; i<sub_size[process_id]; i++) {
			for(int j =0; j<bmpInfo.biWidth ; j++){
				...
				if (size == 1) {
					...
					} else if (i == sub_size[process_id] - 1) {
